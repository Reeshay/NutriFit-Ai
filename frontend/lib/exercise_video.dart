...
class _ExerciseVideoState extends State<ExerciseVideo> {
  ...
  debugPrint("_stopRecording called. isRecording=$_isRecording, "
      "controllerRecording=${_cameraController?.value.isRecordingVideo}");
  ...
  try {
    ...
    // KEY FIX: reset state so UI/button doesn't get stuck,
    // and don't attempt to upload a broken/non-existent recording
    setState(() {
      _isRecording   = false;
      _isUploading   = false;
      _statusMessage = "Recording failed — camera was reset. Please try again.";
    });
  }
}
  ...
  try {
    final uri = Uri.parse("https://nutrifit-backend-production-1761.up.railway.app/exercise_video");
    //final uri = Uri.parse("http://192.168.1.163:8000/exercise_video");
...
print("File size: ${await videoFile.length()}");
...
client.close();
    ...
    // SAVE TO FIRESTORE HERE (skip if 0 reps detected)
    ...
    if (reps > 0 && uid != null) {
      await FirebaseFirestore.instance
        ...
        "duration": (data["duration"] ?? duration).toDouble(),
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_statusMessage == "done" && _suggestion.isEmpty && _poseLabel.isNotEmpty) ...[
  const SizedBox(height: 8),
  Row(
    children: const [
          ...
          Positioned(
            ...
            child: Center(
              child: GestureDetector(
               onTap: () async {
  debugPrint("TAP detected. isBusy=$_isBusy, isUploading=$_isUploading, isRecording=$_isRecording");
