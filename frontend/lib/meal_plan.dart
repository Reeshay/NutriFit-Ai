...
class _MealPlanState extends State<MealPlan> {
  bool loading = true;
  bool _loadingCompensation = false;
  String? errorMessage;
...
Future<void> fetchMealPlanFromFirestore() async {
  ...
  try {
    ...
    try {
      ...
      // cache mein nahi mila toh server se
    ...
    if (doc.exists && doc.data() != null) {
      final raw = (doc.data() as Map<String, dynamic>)['plan'];
      if (raw != null) {
        final planData = Map<String, dynamic>.from(raw as Map);
    ...
    setState(() {
      errorMessage = "No meal plan found.";
      loading = false;
    });
...
Row(
  ...
  children: [
    ...
    ElevatedButton(
      onPressed: () {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (_) => const CheatMealScreen(autoSave: false,),
          ),
        );
      },
    ...
    ElevatedButton(
      onPressed: _loadingCompensation ? null : () async {
        setState(() => _loadingCompensation = true);
        try {
        ...
        Navigator.push(
          ...
          if (mounted) setState(() => _loadingCompensation = false);
        }
      },
      style: ElevatedButton.styleFrom(
        ...
        children: [
          Text(item['food_name'],
              style: const TextStyle(
                  ...
                  color: Colors.black)),
          const SizedBox(height: 8),
          _foodDetail(Icons.scale, "${item['quantity']} ${item['unit']}", "Qty", Colors.blue),
          _foodDetail(
