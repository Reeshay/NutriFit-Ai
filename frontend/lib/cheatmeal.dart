...
class CheatMealScreen extends StatefulWidget {
  ...
  const CheatMealScreen({
    ...
    this.protein,
    this.carbs,
    this.fats, required bool autoSave,
  });
...
class _CheatMealScreenState extends State<CheatMealScreen> {
  ...
  bool saving = false;
  bool _loadingViewPlan = false;

  static const String _backendUrl = 'https://nutrifit-backend-production-1761.up.railway.app/cheatmeal';
...
Future<Map<String, dynamic>?> _fetchUserProfile(String uid) async {
  ...
  DocumentSnapshot mealPlanDoc;
  try {
    mealPlanDoc = await FirebaseFirestore.instance
        .collection('users')
        .doc(uid)
    ...
    print("CACHE MISS, TRYING SERVER");
    mealPlanDoc = await FirebaseFirestore.instance
        .collection('users')
        .doc(uid)
        .collection('meal_plan')
  ...
  if (mealPlanDoc.exists) {
    final plan = mealPlanDoc.data() as Map<String, dynamic>?;
    final planMap = plan?['plan'] as Map<String, dynamic>? ?? {};
    final firstDay = planMap.values.isNotEmpty ? planMap.values.first : null;
  ...
  TextButton(
    ...
    onPressed: _loadingViewPlan ? null : () async {
              final user = FirebaseAuth.instance.currentUser;
              if (user == null) return;

              setState(() => _loadingViewPlan = true);
              try {
              final snapshot = await FirebaseFirestore.instance
                  .collection('users')
                  .doc(user.uid)
              ...
              Navigator.push(
                ...
                if (mounted) setState(() => _loadingViewPlan = false);
              }
            },
          ),
        ],
