...
from meal_plan import ProfessionalMealPlanner, generate_meal_plan_route
# =========================================================
...
class profile_setup:
    ...
    def run(self):
        ...
        # STEP 1: Parse basic numeric fields
        try:
            self.age = int(self.data.get("age", 0))
            if self.age <= 0:
                self.errors["age"] = "Age must be greater than 0"
        ...
        try:
            user_profile_for_meal = {
                ...
                "health_condition": self.data.get("health_condition", []),
                "allergies": self.data.get("allergies", [])
            }


            meal_plan_response = generate_meal_plan_route(user_profile_for_meal)
            if meal_plan_response.get("status") == "success":
        ...
        # ----------------------------
        return self.errors, self.warnings, {**self.get_result(), "meal_plan": meal_plan}
# =========================================================
